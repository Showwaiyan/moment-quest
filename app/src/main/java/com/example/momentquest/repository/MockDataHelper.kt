...
object MockDataHelper {
    ...
    suspend fun initializeMockDataIfNeeded(context: Context): Boolean = withContext(Dispatchers.IO) {
        ...
        val isInitialized = prefs.getBoolean(KEY_MOCK_INITIALIZED, false)
        if (isInitialized) {
            Log.d(TAG, "Mock data already initialized. Skipping.")
            return@withContext false
        }

        Log.d(TAG, "Starting first-time mock data initialization.")
        ...
        db.beginTransaction()
        try {
            // Check if tables are empty before inserting to avoid duplicates
            val cursor = db.query(DatabaseHelper.TABLE_CHALLENGES, arrayOf("id"), null, null, null, null, null)
            ...
            // Mark as initialized
            prefs.edit().putBoolean(KEY_MOCK_INITIALIZED, true).apply()
            Log.d(TAG, "Mock data initialization successful.")
            true
        } catch (e: Exception) {
            Log.e(TAG, "Error during mock data initialization", e)
            false
        } finally {
            db.endTransaction()
            db.close()
